...
// This sis the most complex version of the text helper
...
HelperText.prototype = {
	...
	getTextContent: function (element) {
		...
		var out = this.walkTreeForText(element, isPre)
		if(out !== undefined){
	...
	walkTreeForText: function (element, isPre) {
	    var out = '',
	    	child,
	    	text;
        ...
        if(element.children && element.children.length > 0){
            for (var j = 0; j < element.children.length; j++) {
                ...
                text = this.walkTreeForText(child, isPre);
