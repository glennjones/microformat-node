...
Parser.prototype = {
	...
	get: function(dom, rootNode, options) {
		...
		if(!dom || !rootNode){
			...
			if(this.options.rel){
				...
				out.data.rels = rels.rels;
				out.data['rel-urls'] = rels['rel-urls'];
				if(rels.alternate){
	...
	impliedRules: function(dom, node, uf, parentClasses) {
		...
		function getNameAttr(dom, node) {
			var value = domUtils.getAttrValFromTagList(dom, node, ['img','area'], 'alt');
			if(!value) {
		...
		function getURLAttr(dom, node) {
			var value = null;
			//not with class: include or item or h-item
			if(domUtils.hasAttributeValue(dom, node, 'class', 'include') === false){
					value = domUtils.getAttrValFromTagList(dom, node, ['a'], 'href');
					if(!value) {
						value = domUtils.getAttrValFromTagList(dom, node, ['area'], 'href');
					}
			...
			*/
			...
			if(!uf.properties.name) {
				...
				if(!value) {
					...
					descendant = domUtils.isSingleDescendant(dom, node, ['img', 'area', 'abbr']);
					if(descendant && domUtils.hasAttributeValueByPrefix(dom, descendant, 'class', 'h-') === false){
					...
					if(node.children.length > 0){
						...
						if(child){
							descendant = domUtils.isSingleDescendant(dom, child, ['img', 'area', 'abbr']);
							if(descendant && domUtils.hasAttributeValueByPrefix(dom, descendant, 'class', 'h-') === false){
								value = getNameAttr(dom, descendant);
							}
						}
				...
				if(!value) {
					uf.properties.name = [text.parse(dom, node, textFormat)];
				}else{
					uf.properties.name = [text.textContent(value, textFormat)];
				}
			...
			*/
			...
			if(!uf.properties.url) {
				...
				*/
				...
				if(!value) {
					descendant = domUtils.isOnlySingleDescendantOfType(dom, node, ['a', 'area']);
					if(descendant && domUtils.hasAttributeValueByPrefix(dom, descendant, 'class', 'h-') === false){
					...
					if(node.children.length > 0){
						child = domUtils.isSingleDescendant(dom, node);
						if(child){
							descendant = domUtils.isOnlySingleDescendantOfType(dom, child, ['a', 'area']);
							if(descendant && domUtils.hasAttributeValueByPrefix(dom, descendant, 'class', 'h-') === false){
								value = getURLAttr(dom, descendant);
							}
						}
		...
		if(uf.times.length > 0 && uf.dates.length > 0) {
			newDate = dates.dateTimeUnion(uf.dates[0][1], uf.times[0][1], this.options.dateFormat);
	...
	impliedValueRule: function(uf, parentPropertyName, propertyName, value){
		if(uf.value){
			...
			if(utils.startWith(parentPropertyName,'p-') && propertyName === 'p-name'){
				uf.altValue = {name: propertyName, value: value};
			}
			// if it's an e-* property element
			if(utils.startWith(parentPropertyName,'e-')){
				uf.altValue = {name: propertyName, value: value};
			}
			//f it's a u-* property element
			if(utils.startWith(parentPropertyName,'u-')){
				uf.altValue = {name: propertyName, value: value};
			}
		}
		return uf;
	...
	walkChildren: function(dom, node, out, ufNameArr, rootID, parentClasses) {
		var context = this,
			childOut = {},
			rootItem,
			itemRootID,
		...
		dom(node).children().each(function(i, child) {
			...
			if(classes.root.length === 0 && classes.properties.length > 0) {
				...
				context.walkChildren(dom, child, out, ufNameArr, rootID, classes);
			}
	...
	getDTValue: function(dom, node, className, uf, valueParse) {
		...
		if(out) {
			if(dates.isDuration(out)) {
				...
				if(uf) {
					uf.times.push([className, dates.parseAmPmTime(out, this.options.dateFormat)]);
				}
				return dates.parseAmPmTime(out, this.options.dateFormat);
			} else {
				// returns a date - in a ISO profile format
				if(uf) {
	...
	getValueClass: function(dom, node, propertyType) {
		...
		if(out.length > 0) {
			...
			if(propertyType === 'dt') {
				return dates.concatFragments(out, this.options.dateFormat).toString(this.options.dateFormat);
			}
		} else {
			return null;
		}
	...
	createUfObject: function(names, value) {
		...
		if(value && utils.isOnlyWhiteSpace(value) === false) {
			out.value = value;
		}
			...
			if(relList) {
				...
				while(z < y) {
					...
					if(typeof this.options.baseUrl == 'string' && typeof value === 'string') {
						var resolved = urlParser.resolve(this.options.baseUrl, value);
				...
				if(domUtils.hasAttribute(dom, arr[x], 'href')){
					...
					if(url){
						url = urlParser.resolve(this.options.baseUrl, url );
					}
				}
				...
				if(relList.toLowerCase().indexOf('alternate') > -1){	
					...
					if(items.length > 1){
						if(domUtils.hasAttribute(dom, arr[x], 'rel')){
							...
							obj.rel = utils.trim( clonedRelList.toLowerCase().replace('alternate','') );
						}
					}
					// create the key
					if(!out['alternate']) {
	...
	getRelProperties: function(dom, node){
		...
		if(domUtils.hasAttribute(dom, node, 'media')){
			obj.media = domUtils.getAttribute(dom, node, 'media');
		}
		if(domUtils.hasAttribute(dom, node, 'type')){
			obj.type = domUtils.getAttribute(dom, node, 'type');
		}
		if(domUtils.hasAttribute(dom, node, 'hreflang')){
			obj.hreflang = domUtils.getAttribute(dom, node, 'hreflang');
		}
		if(domUtils.hasAttribute(dom, node, 'title')){
			obj.title = domUtils.getAttribute(dom, node, 'title');
		}
		if(utils.trim(this.getPValue(dom, node, false)) !== ''){
