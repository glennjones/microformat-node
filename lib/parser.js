	...
	if(!url){
		...
		try {
			if (parser.options.cache && parser.options.useCache && parser.options.cache.has(url)) {
				...
				dom = cheerio.load(parser.options.cache.get(url));
				rootNode = dom.root();
		...
		if(uf && uf.properties) {
			...
			*/
			...
			if(!uf.properties.name) {
				...
				if(!value) {
					// single descendant that is a img or abbr i.e. .h-x>img[alt]:only-of-type
					descendant = domUtils.isSingleDescendant(dom, node, ['img', 'abbr']);
					if(descendant){
						value = getNameAttr(dom, descendant);
					}

					// has a single child that has a single descendant that is a img or object i.e. .h-x>:only-child>img[alt]:only-of-type
		...
		dom(node).children().each(function(i, child) {
			...
			if(classes.root.length > 0 && classes.properties.length > 0) {
				...
				rootItem = context.createUfObject(
					classes.root, 
					text.parse(dom, child, this.options.textFormat)
				);
...
// Maybe use - make sure options still work
