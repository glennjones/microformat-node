...
function initParser(options){
  ...
  function parsePlainText(url, options, callback){
    deferred = _.Deferred();
    var promise = deferred.promise();
    ...
    request({uri: url}, function(requestErrors, response, body){
      if(!requestErrors && response.statusCode === 200){
        ...
        var data = helperText.getTextContent($('body')[0]);
  ...
  function parseInnerHTML(url, options, callback){
    deferred = _.Deferred();
    var promise = deferred.promise();
...
exports.parseInnerHTML = parseInnerHTML;
