...
Parser.prototype = {
	...
	impliedRules: function(dom, node, uf) {
		...
		function getURLAttr(dom, node) {
			var value = null;
			//not with class: include or item or h-* (get h-item with prefix exclude)
			if(domUtils.hasAttributeValue(dom, node, 'class', 'include') === false
				&& domUtils.hasAttributeValue(dom, node, 'class', 'item') === false
				&& domUtils.hasAttributeValueByPrefix(dom, node, 'class', 'h-') === false){
			...
			*/
			...
			if(!uf.properties.name) {
				...
				if(value) {
					...
					uf.properties.name = [text.textContent(value, this.options.textFormat)];
				}
			}
