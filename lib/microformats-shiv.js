...
var ufShiv = {
    ...
    get: function (name, element, baseURL) {

        var data = [];
        var nodes = [];
        var uf = this.internal[name];

        ufShiv.baseURL = baseURL;
        ...
        */
        ...
        uriGetter: function (propnode, parentnode) {
            ...
            if (pairs.hasOwnProperty(name)) {
                ...
                var href = propnode.getAttribute(pairs[name]);
                return this.getAbsoluteUrl(ufShiv.baseURL, href);
            }
          ...
          for (var each in bar) {
            if (each in foo) {
              if (typeof(foo[each]) == "object" && typeof(bar[each]) == "object") {
                merged[each] = ufShiv.internal.deepMerge(foo[each], bar[each]);
              } else {
