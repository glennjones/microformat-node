...
function textContent( htmlStr ){
    var text = new Text();
    return text.textContent( htmlStr );
};
...
function Text(){
    this.textFormat = 'normalised'; // normalised or whitespace or whitespacetrimmed
...
Text.prototype = {
    ...
    textContent: function( text, options ){
       if(text){
          var out = '',
              regex = /(<([^>]+)>)/ig;
            
          out = text.replace(regex, "");   
          if(options.textFormat === 'whitespacetrimmed') {    
