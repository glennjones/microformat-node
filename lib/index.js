/*
   ...
   Built: 2024-04-15 05:04 - 
   Copyright (c) 2024 Glenn Jones
   Licensed MIT 
*/
...
var Modules = (function (modules) {
	...
	modules.domUtils = {
		...
		getDOMContext: function( options ){
			...
			if(options.html){
				this.$ = cheerio.load(options.html,{decodeEntities: false});
		...
		getAttrValFromTagList: function(node, tagNames, attributeName) {
			...
			while(i--) {
				if(node.name === tagNames[i]) {
					var attr = this.getAttribute(node, attributeName);
					if(attr && attr !== '') {
						return attr;
					}
				}
	...
	/**
		...
		*/
		createNode: function( tagName ){
			return cheerio.load( '<' + tagName + '></' + tagName + '>',{decodeEntities: false} ).root()[0].children[0];
		},
		...
		*/
		createNodeWithText: function( tagName, text ){
			return cheerio.load( '<' + tagName + '>' + text + '</' + tagName + '>',{decodeEntities: false} ).root()[0].children[0];
		}
