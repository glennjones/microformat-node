...
var entities = require("entities"),
    HelperText = require('../lib/helper-text2.js').HelperText,
...
Uf2Parser.prototype = {
    ...
    getValue: function(dom, node, className, uf){
        ...
        if(this.startWith(className, 'dt-')){
            value = this.getDTValue(dom, node, className, uf)
        }   
        return value;
    },
    ...
    getDTValue: function(dom, node, className, uf){
        var out = this.getValueClass(dom, node, false, true);
        ...
        if(out){
            if(ISODate.isDuration(out)) {
                ...
                uf.times.push([className, ISODate.parseAmPmTime(out)])
                return ISODate.parseAmPmTime(out);
            }else{
                // returns a W3C profile ISO8601 date by adding punctuation
                uf.dates.push([className, new ISODate.ISODate(out).toString()])
