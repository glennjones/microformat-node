...
var UfHelpers = require('../lib/ufhelpers.js').UfHelpers,
    helpers = new UfHelpers();
...
Uf2Parser.prototype = {
    ...
    impliedRules: function(dom, node, uf){
        ...
        if(uf && uf.properties){
            ...
            if(!uf.properties.name){
                ...
                if(!value){
                    value = helpers.getTextContent(node);
                }
                if(value){ 
    ...
    walkChildren: function (dom, node, out){
        ...
        dom(node).children().each(function(i, child){
            ...
            if(classes.root.length > 0 && classes.properties.length > 0){ 
                ...
                rootItem.value = helpers.getTextContent(child);
    ...
    getPValue: function(dom, node){
        ...
        return helpers.getTextContent(node);
    },
    ...
    getValueClass: function(dom, node, normaliseWS){
        ...
        dom(node).children().each(function(i, child){
            if(this.hasClass(dom, child, 'value')){
                ...
                if(!value){
                    value = helpers.getTextContent(child);
                }
                if(value){
