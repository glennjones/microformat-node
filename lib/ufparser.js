...
Uf2Parser.prototype = {
            ...
            while (x < i) {
                ...
                if(uf 
                    ...
                    && this.shouldInclude(uf, filters)
                        ...
                        children = this.findChildItems(dom, items[x], baseURL, uf.type[0])
        ...
        while(x < i){
            ...
            while(y--){
                ...
                if(classList.indexOf(items[y]) > -1){
                    if(foundRootNames.indexOf(items[y]) === -1){
                        out.push(arr[x])
                        foundRootNames.push(this.getV2RootName(items[y]))
                    }
                }
    ...
    walkTree: function(dom, node){
        ...
        if(classes.root.length !== 0){
            // only use first found root
            out = this.createUfObject( classes.root[0] );
            this.walkChildren(dom, node, out, classes.root[0]);
    ...
    walkChildren: function (dom, node, out, ufName){
        var childOut = {},
            context = this;
        ...
        dom(node).children().each(function(i, child){
            ...
            if(classes.root.length > 0 && classes.properties.length > 0){ 
                ...
                var i   = rootItem.type.length,
                    x   = 0;
                while (x < i) {
                    context.walkChildren(dom, child, rootItem, rootItem.type[x]);
                    x++;
                }
                context.impliedRules(dom, child, rootItem);
            }

           // a property which is NOT a microformat
            if(classes.root.length === 0 && classes.properties.length > 0){
                ...
                context.walkChildren(dom, child, out, ufName);
            }
                    ...
                    if(map){
                        for (var key in map.properties) {
                            if(key === item){
                                ...
                                var propName = (prop.map)? prop.map : 'p-' + key;
                                if(prop.uf){
                                    // this property is a root
                                    var y   = 0;
                                    while (y < i) {
