...
var ISODate = require('../lib/isodate.js').ISODate,
...
UfParser.prototype = {
    ...
    get: function($, name, element, baseURL, ownerDocument) {
        var data = [];
        var nodes = [];
        var uf = this[name];
        ...
        if (!uf || !element) {
            return this.pack({}, 'Sorry either the format or element was not found.', this.version);
        }
        ...
        if (name == 'hCard') {
            ...
            for (var i = 0; i < nodes.length; i++) {
                if (this.findParentByClass(nodes[i], 'vcard') === null) {
                    items.push(nodes[i]);
                }
            }
            nodes = items;
        ...
        for (var x = 0; x < nodes.length; x++) {
            data.push(this.getMicroformat(nodes[x], name));
        }
        ...
        return this.pack(obj, '', this.version);
    },
    ...
    pack: function (data, error, version) {
        ...
        pack['parser-information'].name = 'Microformat Node';
