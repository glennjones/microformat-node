            ...
            while (x < i) {
                if(!this.hasAttribute(dom, items[x], 'data-include')){
                    ...
                    while (z < y) {
                        ...
                        if(ufs[z] 
                            && utils.hasProperties(ufs[z].properties)
...
/*        var rootids = dom(node).attr('data-rootids');
    ...
    getUfClassNames: function(dom, node, ufName){
                    ...
                    for (var key in this.maps) {
                        if(this.maps.hasOwnProperty(key)){
                            ...
                            if(this.maps[key].root === item && out.root.indexOf(key) === -1){
                                ...
                                if(this.maps[key].subTree && 
                                    this.isSubTreeRoot(dom, node, this.maps[key], items) === false){
        ...
        var impiedRel = this.findRelImpied(dom, node, ufName);
...
/*        var out = {}
    ...
    findRels: function(dom, rootNode, fromChildren){
        var fromChildren = (fromChildren)? fromChildren : false,
            uf = null;
            out = {};
        ...
        i = arr.length;  
        while(x < i){
            var relList = this.getAttribute(dom, arr[x], 'rel');

            if(relList){
                var items = relList.split(' ')
                var y  = items.length,
                    z   = 0;
                while (z < y) {
                    ...
                    for (var key in this.rels) {
                        if(key === item){
                            var value = this.getAttributeValue(dom, arr[x], ['a', 'area'], 'href');
                            if(!value){
    ...
    findClassIncludes: function(dom, rootNode){
        ...
        while(x < i){
            ...
            var value = this.getAttributeValue(dom, arr[x], ['a'], 'href');
            if(!value){
    ...
    mergeOptions: function(options){
      // single level clone of properties
